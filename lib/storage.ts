...
const KEY='judgement-scorekeeper:v3';
...
export function maxCardsForPlayers(count:number):number{ if(count<=0) return 13; return Math.floor(52/count); }
...
export function computePointsFromOk(bid:number|undefined, ok:boolean|undefined, mode:GameSettings['scoringMode']):number{ if(bid===undefined) return 0; if(!ok) return 0; return mode==='TEN_PLUS_BID'?10+bid:bid; }
export function totalsByPlayer(state:GameState):Record<string,number>{ const totals:Record<string,number>={}; for(const p of state.players) totals[p.id]=0; for(const r of state.rounds){ for(const p of state.players){ const pts=computePointsFromOk(r.bids[p.id], r.ok[p.id], state.settings.scoringMode); totals[p.id]+=pts; } } return totals; }